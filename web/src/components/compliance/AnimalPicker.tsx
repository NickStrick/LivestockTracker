    ...
    <div>
      <div className={clsx("overflow-hidden rounded-xl border bg-surface", error ? "border-danger" : "border-line")}>
        <label className="relative block border-b border-line">
          ...
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search animals" className="h-10 w-full bg-transparent pl-9 pr-3 text-sm outline-none" aria-label="Search animals" />
        </label>
        <ul className="max-h-52 divide-y divide-line overflow-y-auto">
          {rows.length === 0 && <li className="px-3 py-6 text-center text-sm text-muted">No animals available.</li>}
          {rows.map((a) => (
            <li key={a.id}>
      ...
      <p className={clsx("mt-1.5 text-xs", error ? "text-danger" : "text-muted")}>{error ?? `${selected.length} selected`}</p>
    </div>
  );
}
