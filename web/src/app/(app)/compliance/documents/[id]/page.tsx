...
import { DOC_TYPE_FULL, fmtDate, fmtSize } from "@/lib/format";
...
export default async function DocumentPage({ params }: PageProps<"/compliance/documents/[id]">) {
  ...
  return (
    <>
      <PageHeader title={doc.title} subtitle={DOC_TYPE_FULL[doc.doc_type]} back={{ href: "/compliance?tab=documents", label: "Documents" }} actions={<DocStatusBadge status={doc.status} daysLeft={doc.days_left} />} />
      ...
      <div className="grid gap-4 sm:gap-6 lg:grid-cols-3">
        <div className="space-y-4 sm:space-y-6 lg:col-span-2">
          <Card>
            ...
            <dl className="grid grid-cols-2 gap-x-4 gap-y-4 p-4 sm:grid-cols-3 sm:p-5">
              <Field label="Issued" value={fmtDate(doc.issued_at)} />
              <Field label="Expires" value={doc.expires_at ? fmtDate(doc.expires_at) : "Never"} />
              <Field label="Issued by" value={doc.issued_by} />
              <Field label="Ranch" value={ranch ? <Link href={`/ranches/${ranch.id}`} className="text-primary hover:underline">{ranch.name}</Link> : null} />
              <Field label="Movement" value={movement ? <Link href={`/compliance/movements/${movement.id}`} className="text-primary hover:underline">{movement.origin.split(",")[0]} → {movement.destination.split(",")[0]}</Link> : null} />
              <Field label="File" value={`${doc.file_name} (${fmtSize(doc.size_kb)})`} />
            </dl>
            {doc.expires_at && (
              <div className="border-t border-line px-4 py-4 sm:px-5">
                ...
                <div className="h-2 overflow-hidden rounded-full bg-surface2" role="progressbar" aria-valuenow={Math.round(used)} aria-valuemin={0} aria-valuemax={100} aria-label="Validity used">
          ...
          <Card>
            <CardHeader title={`Applies to ${linked.length} ${linked.length === 1 ? "animal" : "animals"}`} icon={faCow} />
        ...
        <Card className="h-fit">
          ...
          <div className="flex flex-col items-center gap-3 p-6 text-center">
            ...
            <p className="text-xs text-muted">Files will download from S3 through a short-lived signed URL once the backend is connected.</p>
          </div>
        </Card>
      </div>
    </>
